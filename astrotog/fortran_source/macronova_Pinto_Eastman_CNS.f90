MODULE macronova_Pinto_eastman_CNS
  ...
  SUBROUTINE macronova(np, parameters, read_hrate, heating_rates_file, Nt, luminosity)
      ...
      DOUBLE PRECISION t0,t1,m_ej,v_med,alpha,e_th,DZ_factor,kappa,Temp0
      DOUBLE PRECISION v_max,rho0,E0,F0,tau0,dt,dx,dx2,dx3,L0
...
!            WRITE(uLum,'(4(ES14.7,1X))') tm*t0, Lum, y, x
         ...
         DO m=1,mmax
            phi(m)= (phi_p(m) * (1.0 - pm(m)*tm_p*0.5*dt)  &
                 + dt*qm(m)*(tm_p + 0.5*dt) &
                *heating_rate(e_th,alpha,tm_p + 0.5*dt, t0, DZ_factor,read_hrate, nLinesHrate,HR, t_HR)) &
                 / (1.0 + pm(m)*tm*0.5*dt)
         ENDDO
         tm_p= tm
         phi_p(1:mmax)= phi(1:mmax)
