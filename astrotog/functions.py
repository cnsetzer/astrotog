...
def observe(table_columns, transient, survey):
    ...
    positional_overlaps = survey.cadence.query(
        "({3} - {0} <= {1} <= {3} + {0}) & ({4} - {0} <= {2} <= {4} + {0})".format(
            survey.FOV_radius,
            transient.ra,
            transient.dec,
            survey.col_ra,
    ...
    for index, row in t_overlaps.iterrows():
        ...
        angdist = np.arccos(
            np.sin(pointing_dec) * np.sin(transient.dec)
            + np.cos(pointing_dec)
            * np.cos(transient.dec)
            * np.cos(transient.ra - pointing_ra)
        )
    ...
    if not overlap_indices:
        ...
        for index, row in survey_overlap.iterrows():
            ...
            source_bandflux = bandflux(
                survey.throughputs[band], SED_model=transient.model, phase=obs_phase
            )
            source_mag = flux_to_mag(
                source_bandflux, survey.reference_flux_response[band]
            )
            extinct_mag = source_mag + A_x
            extinct_bandflux = mag_to_flux(
                extinct_mag, survey.reference_flux_response[band]
            )
            fivesigma = row["fiveSigmaDepth"]
            flux_error = bandflux_error(fivesigma, survey.reference_flux_response[band])
            source_mag_error = magnitude_error(
                source_bandflux, flux_error, survey.reference_flux_response[band]
            )
            extinct_mag_error = magnitude_error(
                extinct_bandflux, flux_error, survey.reference_flux_response[band]
            )
            ...
            inst_mag_error = magnitude_error(
                inst_flux, flux_error, survey.reference_flux_response[band]
            )
        ...
        if (
            positional_overlaps.query("expMJD < {0} - 1.0".format(transient.t0))
            .dropna()
            .empty
        ):
        ...
        if (
            positional_overlaps.query("expMJD > {0} + 1.0".format(transient.tmax))
            .dropna()
            .empty
        ):
...
def write_params(table_columns, transient):
    ...
    if transient.num_params > 0:
        for i in range(transient.num_params):
            pandas_df.at[
                0, getattr(transient, "param{0}_name".format(i + 1))
            ] = getattr(transient, "param{0}".format(i + 1))
...
def detect(pandas_obs_df, filter_dict):
    ...
    for filter, properties in filter_dict.items():
        ...
        if type == "value":
            detected_inter = filter_on_value(
                detected_inter, name, value, gt_lt_eq, absolute
            )
        elif type == "count":
            detected_inter = filter_on_count(
                detected_inter, count, name, value, gt_lt_eq, absolute
            )
        elif type == "both":
            detected_inter = filter_on_count(
                detected_inter, count, name, value, gt_lt_eq, absolute
...
def param_observe_detect(param_df, obs_df=None, detect_df=None):
    if obs_df is not None:
        ...
        for iter, row in param_df.iterrows():
            if (
                obs_df.query("{} == transient_id".format(row["transient_id"])).empty
                is False
            ):
            ...
            if (
                obs_df.query(
                    "{} == transient_id & alert == True".format(row["transient_id"])
                ).empty
                is False
            ):
    ...
    if detect_df is not None:
        ...
        for iter, row in param_df.iterrows():
            if (
                detect_df.query("{} == transient_id".format(row["transient_id"])).empty
                is False
            ):
...
def filter_on_count(
    pandas_df, num_count, filter_column=None, value=None, gt_lt_eq=None, absolute=None
):
    """
    ...
    count_df = pd.Dataframe(
        columns=["counts"], index=list(pandas_df["transient_id"].unique())
    )
    passed_transients = []
    if value and gt_lt_eq:
        value_filtered_obs = filter_on_value(
            pandas_df, filter_column, value, gt_lt_eq, absolute
        )
        for iter, row in value_filtered_obs.iterrows():
            count_df.loc["{}".format(row["transient_id"])] += 1
            if (
                count_df.loc["{}".format(row["transient_id"])] >= num_count
                and not row["transient_id"] in passed_transients
            ):
...
def other_observations(survey, param_df, t_before, t_after, other_obs_columns):
    ...
    positional_overlaps = survey.cadence.query(
        "({3} - {0} <= {1} <= {3} + {0}) & ({4} - {0} <= {2} <= {4} + {0})".format(
            survey.FOV_radius, ra, dec, survey.col_ra, survey.col_dec
        )
    )
    # Split the computaiton of time overlaps into two queries
    t_before_overlaps = positional_overlaps.query(
        "{0} - {1} <= expMJD & expMJD <= {0}".format(t0, t_before)
    )
    t_after_overlaps = positional_overlaps.query(
        "{0} <= expMJD & expMJD <= {0} + {1}".format(tmax, t_after)
    )
    ...
    for index, row in t_overlaps.iterrows():
        ...
        pointing_dec = row[survey.col_dec]
        angdist = np.arccos(
            np.sin(pointing_dec) * np.sin(dec)
            + np.cos(pointing_dec) * np.cos(dec) * np.cos(ra - pointing_ra)
        )
        if angdist < survey.FOV_radius:
            overlap_indices.append(index)
    if not overlap_indices:
            ...
            for iter, row in band_df.iterrows():
                if iter in coadded_indicies:
                    ...
                    if len(same_night_df.index) > 1:
                        ...
                        coadding_series["extincted_flux"] = same_night_df[
                            "extincted_flux"
                        ].mean()
                        coadding_series["source_flux"] = same_night_df[
                            "source_flux"
                        ].mean()
                        coadding_series["instrument_flux"] = same_night_df[
                            "instrument_flux"
                        ].mean()
                        coadding_series["instrument_flux_one_sigma"] = np.sqrt(
                            np.sum(
                                np.square(same_night_df["instrument_flux_one_sigma"])
                            )
                        ) / len(same_night_df.index)
                        coadding_series["source_flux_one_sigma"] = np.sqrt(
                            np.sum(np.square(same_night_df["source_flux_one_sigma"]))
                        ) / len(same_night_df.index)
                        coadding_series["extincted_flux_one_sigma"] = np.sqrt(
                            np.sum(np.square(same_night_df["extincted_flux_one_sigma"]))
                        ) / len(same_night_df.index)
                        coadding_series["airmass"] = same_night_df["airmass"].mean()
                        coadding_series["five_sigma_depth"] = same_night_df[
                            "five_sigma_depth"
                        ].mean()
                        coadding_series["lightcurve_phase"] = same_night_df[
                            "lightcurve_phase"
                        ].mean()
                        coadding_series["extincted_magnitude"] = flux_to_mag(
                            coadding_series["extincted_flux"],
                            survey.reference_flux_response[band],
                        )
                        coadding_series["source_magnitude"] = flux_to_mag(
                            coadding_series["source_flux"],
                            survey.reference_flux_response[band],
                        )
                        coadding_series["instrument_magnitude"] = flux_to_mag(
                            coadding_series["instrument_flux"],
                            survey.reference_flux_response[band],
                        )
                        coadding_series["instrument_mag_one_sigma"] = magnitude_error(
                            coadding_series["instrument_flux"],
                            coadding_series["instrument_flux_one_sigma"],
                            survey.reference_flux_response[band],
                        )
                        coadding_series["extincted_mag_one_sigma"] = magnitude_error(
                            coadding_series["extincted_flux"],
                            coadding_series["extincted_flux_one_sigma"],
                            survey.reference_flux_response[band],
                        )
                        coadding_series["source_mag_one_sigma"] = magnitude_error(
                            coadding_series["source_flux"],
                            coadding_series["source_flux_one_sigma"],
                            survey.reference_flux_response[band],
                        )
                        coadding_series["signal_to_noise"] = (
                            coadding_series["instrument_flux"]
                            / coadding_series["instrument_flux_one_sigma"]
                        )
                    ...
                    coadded_df = coadded_df.append(
                        coadding_series, ignore_index=True, sort=False
                    )
...
def redshift_distribution(param_df, simulation):
    ...
    if is_detected is False:
        ...
        max_depth_detect = list(
            param_df[param_df["true_redshift"] <= max(detect_zs)]["true_redshift"]
        )
    ...
    print(
        "The redshift range of all sources is {0:.4f} to {1:.4f}.".format(
            min(all_zs), max(all_zs)
        )
    )
    print(
        "The redshift range of the detected sources is {0:.4f} to {1:.4f}.".format(
            min(detect_zs), max(detect_zs)
        )
    )
    print(
        "There are {0} detected transients out of {1}, which is an efficiency of {2:2.2f}%  of the total simulated number.".format(
            len(detect_zs), len(all_zs), total_eff
        )
    )
    print(
        "However, this is an efficiency of {0:2.2f}%  of the total that occur within the range that was detected by {1}.".format(
            max_depth_eff, simulation.instrument
        )
    )
    ...
    plt.ylabel(r"$N(z)$")
    plt.title(
        "Redshift Distribution ({0:.2f} bins, {1:2.1f}%  detected depth efficiency.)".format(
            bin_size, max_depth_eff
        )
    )
    return N_z_dist_fig
...
def determine_ddf_transients(simulation, params):
    ...
    cadence = oss.OpSimOutput.fromOpSimDB(
        path, subset=flag, opsimversion=vers, filterNull=filt_null
    ).summary

    if re.search("minion", path) is not None:
        field_key = "fieldID"
    elif (
        re.search("kraken_2042|kraken_2044|nexus_2097|mothra_2049|alt_sched", path)
        is not None
    ):
        field_key = "fieldRA"
    else:
    ...
    for field in list(cadence[field_key].unique()):
        if re.search("minion", path) is None:
            ddf_ra.append(
                np.deg2rad(
                    cadence.query("{0} == {1}".format(field_key, field))["fieldRA"]
                    .unique()
                    .values
                )
            )
            ddf_dec.append(
                np.deg2rad(
                    cadence.query("{0} == {1}".format(field_key, field))["fieldDec"]
                    .unique()
                    .values
                )
            )
    ...
    for i in range(num_ddf_fields):
        ...
        inter1 = params.query(
            "ra - {0} <= {1} & {0} - ra <= {1}".format(field_ra, field_rad)
        )
        inter2 = inter1.query(
            "dec - {0} <= {1} & {0} - dec <= {1}".format(field_dec, field_rad)
        )
        for index, row in inter2.iterrows():
            ra = row["ra"]
            dec = row["dec"]
            angdist = np.arccos(
                np.sin(field_dec) * np.sin(dec)
                + np.cos(field_dec) * np.cos(dec) * np.cos(ra - field_ra)
            )
