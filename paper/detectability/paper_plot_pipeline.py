...
for i, results_folder in enumerate(os.listdir(sim_results_folder)):
    ...
    fig, ax = plt.subplots()
    ax.hist(
        max_snr, bins=20, weights=tot_alert_prob, histtype="bar", log=True, color="k"
    )
    ...
    sldnc['Num"lc_band'] = sldnc.grouby(["transient_id", "bandfilter"]).transform(len)
    ...
    cldnc['Num"lc_band'] = cldnc.grouby(["transient_id", "bandfilter"]).transform(len)
