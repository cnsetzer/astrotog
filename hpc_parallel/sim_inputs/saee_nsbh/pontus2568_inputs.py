...
same_dist = True  # Flag to set largest distribution so that this is equal over all cadences
min_dec = np.deg2rad(-90.0)
max_dec = np.deg2rad(30.0)
transient_duration = 50.0  # in days used to select time before survey to begin injecting transients
t_before = 40.0
t_after = 40.0
...
cadence_path = '/share/data1/csetzer/lsst_cadences/pontus_2568.db'
dither_path = '/share/data1/csetzer/lsst_cadences/descDithers_pontus_2568.csv'
cadence_ra_col = '_ra'
...
run_dir = 'lsst_saee_nsbh_pontus2568_' + datetime.datetime.now().strftime('%d%m%y_%H%M%S')
